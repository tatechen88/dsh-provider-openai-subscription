...
- 在侧边栏底部显示**图标**指示器（不显示文字，窄屏与远程操作都不会溢出）；点击图标才展开数据卡片，卡片按视口夹取并换行显示。图标本身可拖拽到任意位置并记住位置，双击复位。
...
侧边栏底部只有一个指示器：**常驻一个柱状图图标**，点击图标才在它上方展开数据卡片（Esc、再点一次、或卡片上的 × 都能收起）。卡片按视口宽度夹取并换行，因此在手机宽度或远程操作时不会溢出屏幕。内容按当前会话选中的 Provider 切换：

| 当前 Provider | 卡片里的数据 |
|---|---|
| `openai-subscription` | `OpenAI (ChatGPT OAuth)`、各额度窗口的剩余百分比、本会话/今日/本月 token |
| `deepseek-official` | `DeepSeek`、官方余额、账号类型、三个周期的 token 与金额、价格来源 |
| 其他 | 图标不显示 |

鼠标悬停在图标上仍会给出同一份数据的单行摘要，两份内容来自同一个函数，不会互相矛盾。
